...
// 测试API延迟并选择最快的五个数据源
function testApiLatency() {
    // 显示加载提示
    showLoading('正在测试API延迟...');

    // 获取所有非成人API源
    const apiSources = Object.keys(API_SITES).filter(apiKey => {
        const api = API_SITES[apiKey];
        return !api.adult; // 排除成人内容
    });

    // 自定义API处理
    const customApiCount = customAPIs.length;
    for (let i = 0; i < customApiCount; i++) {
        if (!customAPIs[i].isAdult) { // 排除成人自定义API
            apiSources.push('custom_' + i);
        }
    }
    ...
    if (apiSources.length === 0) {
        hideLoading();
        showToast('没有可用的API源', 'error');
        return;
    }

    // 测试每个API源的延迟
    const latencyTests = apiSources.map(async apiId => {
        try {
            let apiUrl;
            ...
            if (apiId.startsWith('custom_')) {
                ...
                apiUrl = customApi.url + API_CONFIG.search.path + 'test';
            } else {
                // 内置API
                if (!API_SITES[apiId]) return null;
                apiUrl = API_SITES[apiId].api + API_CONFIG.search.path + 'test';
            }
            ...
            console.error(`测试API ${apiId} 延迟失败:`, error);
            return null;
        }
    });

    // 等待所有测试完成
    Promise.all(latencyTests).then(results => {
        hideLoading();
        ...
        // 选择最快的五个API
        const fastestFive = validResults.slice(0, 5);
        const fastestApiIds = fastestFive.map(result => result.apiId);

        // 更新选中的API
        selectedAPIs = fastestApiIds;
        localStorage.setItem('selectedAPIs', JSON.stringify(selectedAPIs));
        ...
        // 显示结果
        let toastMessage = '已选择最快的五个API源';
        fastestFive.forEach((result, index) => {
            let apiName;
            if (result.apiId.startsWith('custom_')) {
                const customIndex = result.apiId.replace('custom_', '');
                const customApi = getCustomApiInfo(customIndex);
                apiName = customApi ? customApi.name : '自定义API';
            } else {
                apiName = API_SITES[result.apiId] ? API_SITES[result.apiId].name : result.apiId;
            }
            toastMessage += `\n${index + 1}. ${apiName} (${Math.round(result.latency)}ms)`;
        });

        showToast(toastMessage, 'success', 5000);
    }).catch(error => {
        hideLoading();
        console.error('API延迟测试出错:', error);
        showToast('API延迟测试出错: ' + error.message, 'error');
    });
}
