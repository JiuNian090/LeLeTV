...
- **多维筛选**：类型流派、年份（可折叠）、评分、语言、地区、剧集状态，整块筛选区亦可一键收起
...
- 启动体验：冷启动全程黑底无白屏，启动屏显示真实加载进度环；添加到主屏后（含 iPhone / iPad）启动画面同样为黑底 Logo
- 版本更新自动检测，提示后重载生效
...
├── scripts/                    # 版本生成、打包、启动图生成、钩子安装、技能索引同步脚本
...
### v3.6.4 (2026-09-21)
- 🎉 新增 桌面端支持左右方向键切换页面（← 上一页 / → 下一页）
- 🔧 修复 分类标签水平滑动时误触发页面切换的问题，滑动标签不再翻页
- ✨ 优化 页面切换手势阈值调大，需划过更长距离才切换，减少误触
- ✨ 优化 筛选区折叠按钮移到筛选行右端并高亮收起状态，一眼看出筛选条件是否被折叠
