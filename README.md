...
| 代码图谱 | CodeGraph 172 文件 / 4,917 节点 / 19,152 边 |
...
- **折叠式线路切换面板**：播放页内展开即可切换资源，标题显示当前播放源名称，渐进式并发测速并显示各线路延迟，不必离开当前页面
...
- 启动体验：冷启动全程黑底无白屏，启动屏显示真实加载进度环；添加到主屏后（含 iPhone / iPad，横竖屏均可）启动画面同样为黑底 Logo
- 版本更新自动检测，提示后重载生效
...
| 机制 | 说明 |
|------|------|
| **设备指纹** | 本地随机生成的设备实例 ID 作为主标识；浏览器特征（UA、screen、时区、语言等）叠加设备名生成的软指纹，用于本地数据被清后找回设备身份 |
...
版本号保存在 `VERSION.txt`（当前 `v3.6.6`），`npm run build` 时由 `scripts/generate-version.mjs` 完成：
...
### v3.6.6 (2026-09-19)
- 🎉 新增 iPad 添加到主屏后的启动画面适配，横屏、竖屏均显示黑底 Logo
- 🔧 修复 iPhone 添加到主屏后横屏启动时启动画面空白的问题
- 🔧 修复 个别机型或开启「显示缩放」时启动画面全白、缺少 Logo 的问题，未适配的尺寸也能回退成黑底 Logo
...
| 工具 | 能力 | 规模 |
|------|------|------|
| **CodeGraph** | 符号查询、调用链、影响范围 | 172 文件 / 4,917 节点 / 19,152 边 |
